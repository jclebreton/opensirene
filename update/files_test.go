...
func Test_getZipListFromScratch(t *testing.T) {
	url := ""
	dest := ""
	zipList, err := getZipListFromScratch("Jan", url, dest)
...
func Test_getIncrementalFile(t *testing.T) {
	file := getIncrementalFile(time.Now(), "/tmp")
